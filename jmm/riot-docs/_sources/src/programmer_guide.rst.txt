...
.. figure:: figures/amr_mesh_standalone.png
   :alt: A block-structured AMR mesh
   :align: center
   :width: 800px
...
<https://parthenon-hpc-lab.github.io/parthenon/develop/index.html>`_
...
example, a :math:`256\times256` mesh with :math:`64\times64` blocks is
tiled into :math:`16` MeshBlocks.
...
exchange also prolongates (coarse :math:`\to` fine) and restricts
(fine :math:`\to` coarse). Loop bounds are expressed through
...
is split into :math:`2^{d}` children (:math:`d` = dimensionality),
...
Parthenon exposes its field data in three layers, and RIOT kernels
operate over the outermost one:
...
(Chapter :ref:`chap:loops`) are built on exactly this pack-of-blocks
structure.
...
.. list-table::
   ...
   * - Sparse
     - May be allocated only on the blocks where it is needed (Section :ref:`sec:sparsity`).
   * - Restart
...
The parameters most RIOT users set are collected below. The
authoritative defaults are those read in the Parthenon source; the
table lists the ones a typical run touches.
...
For adaptive runs, each ``<parthenon/refinement``\ :math:`N`\ ``>``
...
A :math:`256\times256` two-dimensional mesh tiled into
:math:`64\times64` blocks, with two levels of adaptive refinement
triggered on the density field:
...
RIOT’s material thermodynamics (Chapter :ref:`chap:materials`). It
...
closure. For more details see the `Singularity-EOS documentation <https://lanl.github.io/singularity-eos/main/index.html>`_
...
this type list plus the ionization “:math:`Z`-split” modifiers, and
...
taking :math:`(\rho,T)` and one taking :math:`(\rho,e)` — reflecting
...
tabular EOS to cache the last :math:`(\log\rho,\log T)` bracket (and,
in RIOT, to inject the ionization state :math:`\bar Z`). RIOT probes
...
.. list-table::
   ...
   * - InternalEnergyFromDensityTemperature
     - Specific internal energy :math:`e` from :math:`(\rho,T)`.
   * - TemperatureFromDensityInternalEnergy
     - Temperature :math:`T` from :math:`(\rho,e)`.
   * - SpecificHeatFrom…
     - Specific heat :math:`c_v`.
   * - BulkModulusFrom…
     - Bulk modulus :math:`B`.
   * - GruneisenParamFrom…
     - Grüneisen parameter :math:`\Gamma`.
   * - DensityEnergyFromPressureTemperature
     - Inverse: :math:`(\rho,e)` from :math:`(p,T)` (by reference).
   * - FillEos
     - Fills a requested subset of :math:`\{p,e,c_v,B,\dots\}` in one call.

The two “off-axis” conversions — :math:`(\rho,e)\to T` via
``TemperatureFromDensityInternalEnergy`` and :math:`(\rho,T)\to e` via
...
Chapter :ref:`chap:hydro`. ``singularity-eos`` provides this in
...
.. list-table::
   ...
   * - PTESolverFixedP
     - Pressure held fixed.
   * - PTESolverPT
     - Solves in :math:`(p,T)` space; two equations regardless of material count.
   * - PTESolverRhoU
     - Per-material density and energy (:math:`2N` equations).
...
into the per-material indexers (equilibrated :math:`\rho_m`,
:math:`f_m`, :math:`e_m`, :math:`T_m`, :math:`p_m`); the returned
...
For *each* material the ``<material``\ :math:`N`\ ``/opac>`` block
...
coefficients (Chapter :ref:`chap:radtransport`).

Opacity Calls
``````````````

The two accessors RIOT calls are the group-mean absorption and
scattering coefficients, both with units of inverse length. Because
the opacity is multigroup, the third argument is an integer *group
index* :math:`g\in[0,N_g)` — *not* a frequency — and each call returns
the mean coefficient for that group at the material’s density and
temperature:
...
(``Rosseland­Group­Absorption­Coefficient``,
``Planck­Group­Absorption­Coefficient``) fix it explicitly. The mean
tables were produced from the underlying monochromatic model at build
time (a ``Gray`` model gives a coefficient :math:`\rho_m\kappa`
independent of frequency; ``PowerLaw`` scales with :math:`\rho_m`,
...
returns :math:`N_g`, ``Get­Group­Bounds()`` returns the :math:`N_g+1`
group edges, and ``Group­Of­Nu(nu)`` maps a frequency to its group
...
RIOT’s radiation solver (Chapter :ref:`chap:radtransport`) is written
to be multigroup: it loops over the runtime group count :math:`N_g`
and evaluates each material’s opacity at the current group index,
exactly as above. A gray run is just the :math:`N_g=1` configuration
of that same code path — only :math:`N_g` and the group bounds change
— so no separate gray API is needed.
...
(Chapter :ref:`chap:regions`) and a handful of boundary sweeps are
...
cells :math:`(k,j,i)` of the current block. A kernel is therefore an
...
the loop-order tag (Section :ref:`chap:loops`, below), this determines
...
Section :ref:`sec:loops-practice`), the block count of the pack, the
...
The nesting order of the block, per-block “variable” work, and the
cell iteration is named by a ``loop_tag``. The letters stand for
**b**\ lock, **v**\ ariable, **o**\ uter cell-chunk, and **i**\ nner
cell (the table below). RIOT does not pick a tag by hand: it selects
one *by backend*, in ``RiotUtils::GetLoopTag``
(``riot_utils/riot_loops.hpp``). On a GPU build it uses ``boiv`` —
point-wise parallelism, one cell per thread — which is the performant
device shape; on a CPU build it uses ``bvoi`` — one block per
``outer`` step with a vectorized inner sweep. A loop carrying the
``SingleBlock`` constraint (Section :ref:`chap:loops`) overrides this
and uses ``boiv`` on *both* backends, so the lone block’s cells still
spread across threads. The intermediate ``bovi`` tag exists in the
abstraction but RIOT does not currently select it.
...
.. list-table::
   ...
   * - Tag
     ...
     - Meaning
   * - bvoi
     - block :math:`\to` var :math:`\to` outer :math:`\to` inner
     - ``outer`` spans blocks only; ``inner`` covers the whole cell range of a block. The CPU path RIOT uses.
   * - bovi
     - block :math:`\to` outer :math:`\to` var :math:`\to` inner
     - ``outer`` spans blocks and cell-chunks; ``inner`` runs over one chunk. Available but unused by RIOT.
   * - boiv
     - block :math:`\to` outer :math:`\to` inner :math:`\to` var
     - Point-wise limit (chunk size :math:`1`); the inner range carries :math:`(k,j,i)` directly. The GPU path RIOT uses (and the ``SingleBlock`` path on both backends).
...
.. list-table::
   ...
   * - Construct
     ...
     - CPU (``raw``, ``bvoi``)
   * - RiotLoop::outer
     - flat ``RangePolicy`` over blocks :math:`\times` cells; one cell per thread
   ...
   * - RiotLoop::inner
     ...
     - nested :math:`k,j` loops with ``#pragma omp simd`` over :math:`i`
   * - registered scratch
     - per-thread (one cell)
