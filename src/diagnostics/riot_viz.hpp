...
inline std::unordered_map<std::string, VizType> VizTypeMap{{"volume", VizType::volume},
                                                           {"contour", VizType::contour},
                                                           {"slice", VizType::slice},
                                                           {"contour_slice", VizType::contour_slice}};
enum class GradType { none, mag, log_mag };
inline std::unordered_map<std::string, GradType> GradTypeMap{{"none", GradType::none},
  ...
  VectorView3D(std::vector<std::vector<std::vector<T>>> &h) {
    ...
    for (int o = 0; o < h.size(); o++) {
      ...
      for (int i = 0; i < h[o].size(); i++) {
        ...
        curr_in += h[o][i].size();
        for (int ii = 0; ii < h[o][i].size(); ii++) host_data(idx++) = h[o][i][ii];
      }
    }
...
class VectorView2D {
 public:
  ...
  VectorView2D(std::vector<std::vector<T>> &h) {
    ...
    for (int i = 0; i < h.size(); i++) {
      host_offset(i) = curr;
      for (int j = 0; j < h[i].size(); j++) host_data(idx++) = h[i][j];
      curr += h[i].size();
    }
...
class VectorView1D {
 public:
  VectorView1D(std::vector<T> &h) : data("vecview1d", h.size()) {
    auto host_data = data.GetHostMirror();
    for (int i = 0; i < h.size(); i++) host_data(i) = h[i];
    Kokkos::deep_copy(data, host_data);
  }
...
struct RenderingParams {
  ...
  RenderingParams(vec2d_t<VizType> &t, vec2d_t<int> &pack_idx, vec2d_t<int> &alpha_pack_idx,
                  vec2d_t<Real> &range_min, vec2d_t<Real> &range_max, vec2d_t<Real> &alpha_range_min, vec2d_t<Real> &alpha_range_max,
                  vec2d_t<GradType> &use_grad, vec2d_t<GradType> &alpha_use_grad,
                  std::vector<Real> &alpha_tol, vec2d_t<DataBox> &r, vec2d_t<DataBox> &g, vec2d_t<DataBox> &b, vec2d_t<DataBox> &a,
                  vec3d_t<Real> &contours, vec3d_t<Real> &rc, vec3d_t<Real> &gc, vec3d_t<Real> &bc, vec3d_t<Real> &ac,
                  vec3d_t<Real> &slice_x, vec3d_t<Real> &slice_n, std::vector<Real> &xl, std::vector<Real> &yl, std::vector<Real> &zl,
                  std::vector<Real> &ambient, std::vector<Real> &diffuse, vec3d_t<mask_tuple_t> &mask, vec3d_t<int> &mask_id)
    : type(t), pack_idx(pack_idx), alpha_pack_idx(alpha_pack_idx), range_min(range_min), range_max(range_max),
      alpha_range_min(alpha_range_min), alpha_range_max(alpha_range_max), use_grad(use_grad), alpha_use_grad(alpha_use_grad),
      alpha_tol(alpha_tol), r(r), g(g), b(b), a(a),
      contours(contours), rc(rc), gc(gc), bc(bc), ac(ac), slice_x(slice_x), slice_n(slice_n), xl(xl), yl(yl),
      zl(zl), ambient(ambient), diffuse(diffuse), mask(mask), mask_id(mask_id) {
    nlayers = parthenon::ParArray1D<int>("nlayers", t.size());
    auto host_nlayers = nlayers.GetHostMirror();
    for (int i = 0; i < t.size(); i++) host_nlayers(i) = t[i].size();
  ...
  explicit RenderingParams(RenderingParamsVec &rv)
    : RenderingParams(rv.type, rv.pack_idx, rv.alpha_pack_idx, rv.range_min, rv.range_max, rv.alpha_range_min, rv.alpha_range_max,
                      rv.use_grad, rv.alpha_use_grad, rv.alpha_tol, rv.r, rv.g, rv.b, rv.a, rv.contours, rv.rc, rv.gc, rv.bc, rv.ac, rv.slice_x, rv.slice_n, rv.xl,
                      rv.yl, rv.zl, rv.ambient, rv.diffuse, rv.masks, rv.mask_id) {}

  KOKKOS_INLINE_FUNCTION
  PixelVal val_to_rgba(const int outer, const int inner, const Real val, const Real aval) const {
    Real v = (val - range_min(outer, inner)) / (range_max(outer, inner) - range_min(outer, inner));
    Real av = (aval - alpha_range_min(outer, inner)) / (alpha_range_max(outer, inner) - alpha_range_min(outer, inner));
    v = std::clamp(v, 0.0, 1.0);
    av = std::clamp(av, 0.0, 1.0);
    return {r(outer, inner).interpToReal(v),
            g(outer, inner).interpToReal(v),
            b(outer, inner).interpToReal(v),
            a(outer, inner).interpToReal(av)};
  }

  bool masks(const int cam_id, const int layer_id, const Real x, const Real y, const Real z) const {
    bool in = true;
...
class VizData {
 public:
  ...
  : inv_dx(1.0 / composer->template dx<X1DIR>()),
    ...
    if (ilay < r.nlayers(composer->cam_id)) {
      ...
      if (r.alpha_pack_idx(composer->cam_id, ilay) == r.pack_idx(composer->cam_id, ilay)) {
        A_opac = A;
      } else {
        auto val = [&](const int kk, const int jj, const int ii) {
          return composer->vp(composer->cell_b,
                   r.alpha_pack_idx(composer->cam_id, ilay),
                   composer->cell_k + composer->dk * kk,
                   composer->cell_j + composer->dj * jj,
                   composer->cell_i + ii);
        };
 ...
 private:
  ...
  template <typename F>
  KOKKOS_INLINE_FUNCTION
  void set_coeffs(const F &val, std::array<Real, 8> &c) {
    c[0] = val(0, 0, 0);
    c[1] = val(0, 0, 1) - c[0];
    c[2] = val(0, 1, 0) - c[0];
    c[3] = val(1, 0, 0) - c[0];
    ...
    c[7] = val(1, 1, 1) - val(0, 1, 1) - val(1, 0, 1) - val(1, 1, 0) + val(0, 0, 1) + val(0, 1, 0) + val(1, 0, 0) - c[0];
  }
  KOKKOS_INLINE_FUNCTION
  Real interp_(const Real xt, const Real yt, const Real zt, const std::array<Real, 8> &c) const {
    const Real wx = (xt - xlo) * inv_dx;
    const Real wy = (yt - ylo) * inv_dy;
    const Real wz = (zt - zlo) * inv_dz;
    return c[0] + wx * c[1] + wy * (c[2] + wx * c[4]) + wz * (c[3] + wx * c[5] + wy * (c[6] + wx * c[7]));
  };
  KOKKOS_INLINE_FUNCTION
  std::array<Real, 3> gradient_(const Real xt, const Real yt, const Real zt, const std::array<Real, 8> &c) const {
    const Real wx = (xt - xlo) * inv_dx;
    const Real wy = (yt - ylo) * inv_dy;
    const Real wz = (zt - zlo) * inv_dz;
  ...
  Real log_grad_mag_(const Real xt, const Real yt, const Real zt,
    ...
    const Real gx = c[1] + wy * c[4]
                        + wz * (c[5] + wy * c[7]);

    const Real gy = c[2] + wx * c[4]
                        + wz * (c[6] + wx * c[7]);

    const Real gz = c[3] + wx * c[5]
                        + wy * (c[6] + wx * c[7]);
...
class Composer : public RayTrace::IntegratorBase<VarPack_t> {
 public:
  ...
  KOKKOS_FUNCTION
  Composer(VarPack_t &vp, SwarmPack_t &ps, const int b, const int pidx, const int ndim, SwarmPackInt_t &ps_int,
    const RenderingParams &rp)
    : RayTrace::IntegratorBase<VarPack_t>(vp, ps, b, pidx, ndim), rp(rp), cam_id(ps_int(b, vr::camera_id(), pidx)),
      r(ps(b, vr::r(), pidx)), g(ps(b, vr::g(), pidx)), b(ps(b, vr::b(), pidx)), a(ps(b, vr::a(), pidx)) {
    PARTHENON_REQUIRE(rp.nlayers(cam_id) < max_layers, "Too many viz layers requested. Must recompile after setting max_layers large enough");
  }
  ...
  Real get_value(const int ilay) const {
    return get_value(ilay, xc[0], xc[1], xc[2]);
  }
  ...
  Real distance_to_face() const {
    ...
    if constexpr (parthenon::IsCoord<parthenon::UniformCartesian>()) {
      loop_3d([&]<int d>() {
        auto ht = (RayTrace::IntegratorBase<VarPack_t>::template xlo<d>() + (v[d-1] > 0) * RayTrace::IntegratorBase<VarPack_t>::template dx<d>() - xc[d-1]) / v[d-1];
        h = std::min(h, ht);
      });
      ...
      nc[1] = v[2];
      loop_2d([&]<int d>() {
        auto ht = (RayTrace::IntegratorBase<VarPack_t>::template xlo<d>() + (nc[d-1] > 0) * RayTrace::IntegratorBase<VarPack_t>::template dx<d>() - xc[d-1]) / nc[d-1];
        h = std::min(h, ht);
      });
      ...
      auto ht = (RayTrace::IntegratorBase<VarPack_t>::template xlo<X1DIR>() + (nr > 0) * RayTrace::IntegratorBase<VarPack_t>::template dx<X1DIR>() - xc[0]) / nr;
      h = std::min(h, ht);
  ...
  void advance() {
    ...
    if (!initial_mask) {
      ...
      for (int d = 0; d < 3; d++) temp[d] = x[d] + h * v[d];
      bool test = all_mask(temp);

      if (!test) {
        for (int d = 0; d < 3; d++) x[d] = temp[d];
        sync_cart_to_coord();
    ...
    // Loop over layers to initialize data and check for intersections with contours and slices
    int layer_id = -1;
    int contour_id = -1;
    int slice_id = -1;
    for (int ilay = 0; ilay < rp.nlayers(cam_id); ilay++) {
      ...
      if (rp.type(cam_id, ilay) == VizType::slice) {
        Real n_dot_v = rp.slice_n(cam_id, ilay, 0) * v[0] + rp.slice_n(cam_id, ilay, 1) * v[1] + rp.slice_n(cam_id, ilay, 2) * v[2];
        if (std::abs(n_dot_v) > 1.e-14) {
          ...
          Real n_dot_p = rp.slice_n(cam_id, ilay, 0) * xp + rp.slice_n(cam_id, ilay, 1) * yp + rp.slice_n(cam_id, ilay, 2) * zp;
      ...
      else if (rp.type(cam_id, ilay) == VizType::contour || rp.type(cam_id, ilay) == VizType::contour_slice) {
        std::array<Real, 3> final_x;
        if (!mask[ilay]) {
          for (int d = 0; d < 3; d++) final_x[d] = x[d] + h * v[d];
    ...
    // half step
    for (int d = 0; d < 3; d++) x[d] += 0.5 * h * v[d];
    sync_cart_to_coord();
    Real hint = h;
    ...
    if (slice_id >= 0) {
      if (rp.masks(cam_id, slice_id, x[0], x[1], x[2])) {
        ...
        dly /= dist;
        dlz /= dist;
        Real ndotl = rp.slice_n(cam_id, slice_id, 0) * dlx + rp.slice_n(cam_id, slice_id, 1) * dly + rp.slice_n(cam_id, slice_id, 2) * dlz;
        Real lum = rp.ambient(cam_id) + rp.diffuse(cam_id) / (dist * dist) * std::max(0.0, ndotl);
      ...
      if (rp.masks(cam_id, layer_id, x[0], x[1], x[2])) {
        if (rp.type(cam_id, layer_id) == VizType::contour) {
          ...
          Real lum = rp.ambient(cam_id) + rp.diffuse(cam_id) / (dist * dist) * std::max(0.0, -gdotl);
