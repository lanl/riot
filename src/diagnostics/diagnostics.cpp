...
//----------------------------------------------------------------------------------------
//! \fn  void diagnostics::AddDiagnostics
//! \brief
void AddDiagnostics(ParameterInput *pin, Packages_t &packages) {
  if (pin->DoesParameterExist("diagnostics", "packages")) {
    auto diags = pin->GetVector<std::string>("diagnostics", "packages",
                                             "Diagnostic packages to enable at runtime");
    for (auto &diag : diags) {
