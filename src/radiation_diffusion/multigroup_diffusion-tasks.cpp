...
parthenon::TaskStatus MultiGroupTasks<temperature>::InitializeRadiationQuantities(
  ...
  RiotLoop::outer(
      idx_space, KOKKOS_LAMBDA(const lt::idx_range_t &idx_range, const int b) {
        ...
        RiotLoop::inner(idx_range, [&](const int k, const int j, const int i) {
          denom(k, j, i) = cv(k, j, i);
          RT(k, j, i) = Egas(k, j, i) - Egas0(k, j, i);
          for (int g = 0; g < ngroup; ++g) {
            pack_base(b, dSdT(g), k, j, i) = dSdt(g, k, j, i);
            denom(k, j, i) += dSdt(g, k, j, i);
            RT(k, j, i) += S(g, k, j, i);
          }
          pack_base(b, dTc(), k, j, i) = -RT(k, j, i) / denom(k, j, i);
        });
        ...
        RiotLoop::inner(idx_range, [&](const int k, const int j, const int i) {
          for (int g = 0; g < ngroup; ++g) {
            const Real Eg0 = pack_star(b, Egroup(g), k, j, i);
            const Real Eg = pack_base(b, Egroup(g), k, j, i);
            const Real dsdt = dSdt(g, k, j, i);
            Real rhs;
            if (update_temperature) {
              rhs =
                  -(Eg - Eg0 + dsdt / denom(k, j, i) * (Egas(k, j, i) - Egas0(k, j, i)) -
                    cv(k, j, i) / denom(k, j, i) * S(g, k, j, i));
              pack_matrix(b, diag_loc(g), k, j, i) =
                  1.0 +
                  dtcl * aa(g, k, j, i) * ((denom(k, j, i) - dsdt) / denom(k, j, i));
            } else {
              rhs = -(Eg - Eg0 - S(g, k, j, i));
              pack_matrix(b, diag_loc(g), k, j, i) = 1.0 + dtcl * aa(g, k, j, i);
            }
            pack_matrix(b, sigma(g), k, j, i) = dtcl * aa(g, k, j, i) / denom(k, j, i);
            pack_base(b, kappa_cell(g), k, j, i) = atot(g, k, j, i);
            pack_matrix(b, dSdT(g), k, j, i) = dsdt;

            // Add in the inter-group coupling
            if (update_temperature) {
              for (int gp = 0; gp < ngroup; ++gp) {
                if (gp != g) {
                  rhs -= (dSdt(g, k, j, i) * S(gp, k, j, i) -
                          dSdt(gp, k, j, i) * S(g, k, j, i)) /
                         denom(k, j, i);
                }
              }
            }
            pack_rhs(b, Egroup(g), k, j, i) = rhs;
          }
        });
      });
  ...
  for (int dim = 0; dim < pmesh->ndim; ++dim) {
    ...
    RiotLoop::outer(
        idx_space_f, KOKKOS_LAMBDA(const ltf::idx_range_t &idx_range, const int b) {
          RiotLoop::inner(idx_range, [&](const int k, const int j, const int i) {
            for (int g = 0; g < ngroup; ++g) {
              const Real kappa_cellp = pack_base(b, kappa_cell(g), k, j, i);
              const Real kappa_cellm =
                  pack_base(b, kappa_cell(g), k - koff, j - joff, i - ioff);
              // Diffusion coefficients for calculating RHS
              const Real kf = 2.0 / (1.0 / kappa_cellm + 1.0 / kappa_cellp);
              pack_base(b, te, kappa_face(g), k, j, i) = kf;
              pack_matrix(b, te, D(g), k, j, i) = dtcl * dtcl / (3.0 * (1.0 + dtcl * kf));
            }
          });
        });
  }
