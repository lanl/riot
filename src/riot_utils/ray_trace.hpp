...
struct CellInfo {
  ...
  CellInfo(const Coordinates_t &coords, const int b, const int numdim)
    ...
    min_dx = std::min(coords.Dxf<X3DIR>(0), std::min(coords.Dxf<X2DIR>(0), coords.Dxf<X1DIR>(0)));
  }
  ...
  void init_cell(const Real &x1, const Real &x2, const Real &x3) {
    ...
    cell_j = ndim > 1 ? get_index(x2, coords.Xf<X2DIR>(0),
                              coords.Dxf<X2DIR>(0))
                  : 0;
    cell_k = ndim > 2 ? get_index(x3, coords.Xf<X3DIR>(0),
                              coords.Dxf<X3DIR>(0))
                  : 0;
  }
  ...
  KOKKOS_FORCEINLINE_FUNCTION Real xhi() const {
    return coords.Xf<dir>(cell_k + (dir == X3DIR), cell_j + (dir == X2DIR), cell_i + (dir == X1DIR));
  }
  template <int dir>
  ...
  KOKKOS_FORCEINLINE_FUNCTION Real volume() const { return coords.CellVolume(cell_k, cell_j, cell_i); }
...
class IntegratorBase : public CellInfo {
 public:
  ...
  KOKKOS_FUNCTION
  IntegratorBase(VarPack_t &vp, SwarmPack_t &ps, const int b, const int pidx, const int ndim)
    : CellInfo(vp.GetCoordinates(b), b, ndim), vp(vp), pidx(pidx), xc{ps(b, swarm_position::x(), pidx), ps(b, swarm_position::y(), pidx),
      ps(b, swarm_position::z(), pidx)}, v{ps(b, rt::vx(), pidx), ps(b, rt::vy(), pidx), ps(b, rt::vz(), pidx)},
  ...
  template <typename F>
  KOKKOS_INLINE_FUNCTION
  void loop_3d(F &&func) const {
    loop_exec<X1DIR, X3DIR>(func);
  }
  template <typename F>
  KOKKOS_INLINE_FUNCTION
  void loop_2d(F &&func) const {
  ...
  template <typename SwarmPack_t>
  KOKKOS_INLINE_FUNCTION
  void commit(SwarmPack_t &ps, const int b) {
  ...
  void sync_cart_to_coord() {
    ...
    xc[0] = xcoord[0]; xc[1] = xcoord[1]; xc[2] = xcoord[2];
  }
...
template <template <typename> class Integrator_t, bool RemoveComplete, bool MarkRemoval, typename SwarmPack_t, typename VarPack_t, typename... Args>
