...
RIOT currently supports a spatially uniform acceleration directed along a single mesh axis. Writing :math:`g` for the (signed) acceleration and :math:`\hat{\vec{e}}_d` for the chosen axis, the potential is :math:`\Phi = -g\,x_d` so that

.. math::

     \vec{g} = -\nabla\Phi = g\,\hat{\vec{e}}_d ,
