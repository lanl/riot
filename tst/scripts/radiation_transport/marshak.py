...
# Verification test for the nonlinear (equilibrium-diffusion) Marshak wave, solved here
# with SN (discrete-ordinates) transport rather than equilibrium diffusion.
#
# This is the analogue of the radiation_diffusion/marshak verification test.  A
# constant-temperature radiating wall is driven into a cold medium with a T^-3 opacity;
# with fixed_fluid and a diffusion-dominated (optically thick) regime the SN solution
# reproduces the classic Zel'dovich-Raizer nonlinear thermal-conduction wave, whose
# solution is self-similar: the front advances as x_f ~ sqrt(t) and the temperature
# profile collapses onto a single curve in the similarity variable xi = x / sqrt(t).
#
# We verify that self-similarity directly.  We check
#   - profile collapse across output times catches shape/physics breakage;
#   - the front rate x_f/sqrt(t) pins the absolute propagation speed against the analytic
#     equilibrium-diffusion value (SN matches it to <1% in the thick regime).
...
# Max allowed relative spread of T(xi) across output times (self-similarity). SN's front
# is slightly less smooth than equilibrium diffusion (finite angular set + sharp front)
collapse_tol = 3.5e-2

# Analytic front rate x_f/sqrt(t) for the equilibrium-diffusion Marshak wave.  The thick-
# regime SN solution reproduces this closely (measured within <1%).
...
Cv = 8.61733e10  # wave-material specific heat [erg/g/K]
...
# Allowed fractional deviation.  The discretized SN front lags the analytic value by a few
# percent (finite-cell smearing + startup transient + angular discretization); 5% leaves
# margin while still catching a wrong-magnitude (e.g. 2x diffusivity) regression.
front_rate_tol = 0.05
...
def self_similar_collapse(
    ...
    is time-independent.  This is a machine-independent physics check: it compares a run
    to itself across output times, so it is immune to cross-architecture last-bit
    differences and to the sub-cell front-position wiggle that makes a pointwise gold
    comparison fragile.
    ...
    for d in dumps:
        ...
        x = x[order]
        v = v[order]
        # Restrict to the physical wave domain x >= 0.
        keep = x >= 0.0
        x = x[keep]
        v = v[keep]
...
def run(**kwargs):
    ...
    riot.generate("radiation_transport/marshak.py")
    ...
    # the wave material (mat0); Tb is the hot-boundary temperature, which appears both as
    # the driven L-state temperature and the SN incident-blackbody wall.
    args = [
        "parthenon/job/problem_id=" + file_id,
        "material0/Cv=" + repr(Cv),
        "material0/kappa0_a=" + repr(kappa0),
        "problem/rho_l=" + repr(rho),
        "problem/rho_r=" + repr(rho),
        "problem/temp_l=" + repr(Tb),
    ]
    riot.mpirun(nranks, "radiation_transport/marshak.rin", args)
...
def analyze():
    ...
    if len(dump_files) < 3:
        ...
        return False
    dumps = [phdf(f) for f in dump_files]
