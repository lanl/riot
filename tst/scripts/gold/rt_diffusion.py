...
file_id = "rt_diffusion"
gold_id = "rt_diffusion"
...
def run(**kwargs):
    ...
    riot.generate("ionization/rt_diffusion.py")
    logger.debug("Runnning test " + __name__)

    if kwargs["gpu"]:
        # on GPU, run on rank
        nranks = 1
        arguments = ["parthenon/job/problem_id=rt_diffusion", "isotope_data/filename=../../../inputs/tn/dummy_tn_data.hdf5"]
        riot.mpirun(nranks, "ionization/rt_diffusion.rin", arguments)
    else:
...
def analyze():
    ...
    test_dump = "build/src/rt_diffusion.out1.final.phdf"
    gold_dump = "scripts/gold/files/rt_diffusion.out1.final.phdf"
    ...
    if mixed_err > 1.0:
        logger.warning(f"rt_diffusion: max mixed error = {max_err_mixed:23.15e}")
